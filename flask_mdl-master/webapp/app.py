...
#def dt_model(input_variables):
	#print("=====No of estimator===")
	#print(no_of_trees)
	#if no_of_trees == 20:
#	with open(f'model/classifier.pickle', 'rb') as f:
...
#		classfier = RandomForestClassifier(n_estimators=int(no_of_trees), random_state= 0)
...
def main():
	...
	if flask.request.method == 'POST':
		...
		if model_choice == "svm_model":
			...
			model1 = joblib.load(r'model/classifier.pickle')
			#rf_model = pickle.load(model1)

			prediction = model1.predict(input_variables)[0]
		else:
			model = "Random Forest Algorithm"
			prediction = tune_model(input_variables, estimator)#tuning the parameters of the random-forest-model
